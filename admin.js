...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Delegação de eventos para botões de template nos cards de cliente
  document.getElementById('clients-grid').addEventListener('click', e => {
    const btn = e.target.closest('[data-client-id]');
    if (!btn) return;
    abrirModal(btn.dataset.clientId, btn.dataset.clientName, btn.dataset.type);
  });
});
...
async function carregarClientes() {
  ...
  grid.innerHTML = clients.map(client => `
    <div class="client-card">
      <div class="card-initial" aria-hidden="true">${escHtml(client.name.charAt(0).toUpperCase())}</div>
      <div class="card-body">
      ...
      <div class="card-actions">
        <button class="btn-template"
          ...
          data-client-name="${escAttr(client.name)}"
          data-type="header">
          Cabeçalho
        </button>
        <button class="btn-template"
